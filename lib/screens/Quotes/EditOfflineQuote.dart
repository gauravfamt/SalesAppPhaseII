...
class EditOfflineQuote extends StatefulWidget {
  ...
  @override
  _EditOfflineQuoteState createState() => _EditOfflineQuoteState();
}

class _EditOfflineQuoteState extends State<EditOfflineQuote> {
...
//    'Quantity',
  ...
  List<ProductQuantity> selectedProductd;
  ...
  void initState() {
    ...
    ApiService.getAutoSaveQuoteInterval().then((value) {
      ...
      selectedProductd = List<ProductQuantity>();
    }).catchError((err) {
      intervalDuration = new Duration(seconds: intervalValue);
      isFullScreenLoading = false;
      ...
      if (widget.ProductQuantityList.length > 0) {
        if (isProductRealTimeDataLoded == false) {
          ...
          if (isHeaderLoded == isDetailsLoded && isProductLoaded == false) {
            ...
            OtherParam otherParam; //Added by Gaurav Gurav, 31-Aug-2022
            for (int i = 0; i < widget.ProductQuantityList.length; i++) {
              print(
                  'Product :${widget.ProductQuantityList[i].ProductObject.ProductCode}');
    ...
    _standardDropDownFieldsDBHelper
        ...
        .then((value) => {
              if (value.length > 0)
                ...
                {
                  print(
                      'Tax DropDown Entry not present at localDB for addQuoteScreen'),
  ...
  void fetchShippingAddressesValue() async {
    String customerNo = selectedCompany != null
        ...
        : _quote.QuoteHeader.first.QuoteHeaderFields
    ...
    _addressDBHelper
      ...
      if (addressRes.length > 0) {
        print("Quote Header Fetch Test");
        print(_quote.QuoteHeader.first.QuoteHeaderFields
            .firstWhere(
              (element) => element.FieldName == "ShippingAddressCode",
              orElse: () => _quote.QuoteHeader.first.QuoteHeaderFields.first,
            )
            .FieldValue);
        //Attaches Addresses
        if (addressRes.isNotEmpty) {
          ...
          if (selectedCompany != null &&
            ...
            if (!shippingAddresses.containsValue(shippingAddressCode)) {
              shippingAddresses[shippingAddressCode] = shippingAddressCode;
            }
          }
        }
        print("fetchShippingAddressesValue Result");
        print(shippingAddressCode);
...
//                {
              ...
              if (widget.addQuoteType == AddQuoteType.EDIT_OFFLINE_QUOTE)
                ...
                {
                  print('addQuoteType Not sent for the EditOfflineQuote '),
                }
                ...
                if (localAddQuotesRes.length > 0)
                  {
                    ...
                    print(
                        'difference ${_serverUpdatedDate.difference(_localServerUpdatedDate)}'),
      ...
      _addQuoteDBHelper
          ...
          .then((quotesRes) => {
                if (quotesRes.length > 0)
                  {
                    // #region Print Quote
                    quotesRes.first.QuoteHeader.forEach((element) {
                    ...
                    quotesRes.first.QuoteDetail.forEach((element) {
          ...
          .catchError((e) {
        print('Error while binding Offline quote to the EditOfflineQuote ');
        print(e);
        showErrorToast('Try Again Later');
      });
    } catch (e) {
    ...
    try {
      ...
      quoteObj.QuoteHeader.forEach((singleQuoteHeader) {
        singleQuoteHeader.QuoteHeaderFields.forEach((singleField) {
          ...
          } else if (singleField.FieldName == 'ShippingAddressCode') {
            if (_fieldValue != 'null') shippingAddressCode = _fieldValue;
          } else
            singleField.textEditingController =
                new TextEditingController(text: _fieldValue);
        });
...
//              requestBody += '{';
//              requestBody +=
//              '"ProductCode" : "${widget.ProductQuantityList[i].ProductObject
//                  .ProductCode}",';
...
//                print(
//                    'ProductRealTimePriceData Lenght:${ProductRealTimePriceData
//                        .length}'),
//              }
//          })
//              .catchError((e) =>
...
//                        isLoadingLocalQuote: isLoadingLocalQuote,
                  ...
                  else
                    {
                      ...
                      print(
                          'inside addServerQuoteDetailFields ${widget.listingQuoteObj.Quotedetail.length}'),
                      // AddProductData(int.parse(widget.listingQuoteObj.Quotedetails.length.toString())),
                    },
                })
            .catchError((e) => {
  ...
  void addServerQuoteDetailFields({
    ...
    try {
      ...
      if (_temp.length > 0) {
        ...
        print(
            '_temp fields not Present for insertion inside addServerQuoteDetailFields Fn');
        handleAddQuoteError(quoteId: quote.Id);
      }
    } catch (e) {
  ...
  void loadSelectedLookupsData({
    ...
    try {
      ...
      print(
          'quoteSalesSitesCodeNoList Selected SalesSites fetch $quoteSalesSitesCodeNoList');
...
//      },
  ...
  Widget getQuoteHeaderDDF({
    ...
    print("Inside getQuoteHeaderDDF");
    print(selectedValue);
  ...
  void handleCompanyChange({
    ...
    setState(() {
      ...
      _quote
          .QuoteHeader[quoteHeaderPosition]
          .QuoteHeaderFields[quoteHeaderFieldPosition]
          .FieldValue = companyObj.CustomerNo;
    });
  }
        ...
        setState(() {
          ...
          _quote.QuoteDetail[quotePosition].QuoteDetailFields
            ...
            if (singleQuoteElm.FieldName == 'ExtAmount' &&
              ...
              {
              print('-----OtherParam--  ${otherParam.toStringify().toString()}');
...
//      }
      ...
      for (var qdf = 0;
        ...
        QuoteDetailField _singleQuoteDetailField =
            _quote.QuoteDetail[quoteDetailPosition].QuoteDetailFields[qdf];
//
//        if (_singleQuoteDetailField.FieldName == 'BasePrice') {
//          ///IF THE FIELD IS EXT_AMOUNT THE MULTIPLYING QUANTITY BY THE PRODUCT's BASE_PRICE
//          setState(() {
//            _singleQuoteDetailField.textEditingController.text = (_basePrice).toStringAsFixed(2).toString();
//            _singleQuoteDetailField.FieldValue = (_basePrice).toStringAsFixed(2).toString();
//          });
//        }
...
//          style: _formTFTextStyle,
  ...
  void handleCheckAll(bool isCheckedValue) {
    setState(() {
      if (isCheckedValue) {
        ...
        for (int i = 0; i < _quote.QuoteDetail.length; i++) {
          _quote.QuoteDetail[i].product.isSelected = true;
          int Qty = 1;
          for (int j = 0;
              j < _quote.QuoteDetail[i].QuoteDetailFields.length;
              j++) {
          ...
          var data = selectedProductd.firstWhere(
              (element) =>
                  element.ProductObject.ProductCode ==
                  _quote.QuoteDetail[i].product.ProductCode,
              orElse: () => null);
          if (data == null) {
            selectedProductd
                .add(ProductQuantity(_quote.QuoteDetail[i].product, Qty));
          }
        }
      } else {
        isAllProductSelected = false;
        for (int i = 0; i < _quote.QuoteDetail.length; i++) {
          _quote.QuoteDetail[i].product.isSelected = false;
        }
        selectedProductd = List<ProductQuantity>();
      }
    });
  ...
  void handleCheckBoxOnChange(bool isCheckedValue, int rowPosition) {
    setState(() {
      _quote.QuoteDetail[rowPosition].product.isSelected = isCheckedValue;
      if (isCheckedValue) {
        ///ADDING SELECTED QUOTE TO SEND_MAIL_LIST
        int Qty = 1;
        for (int j = 0;
            j < _quote.QuoteDetail[rowPosition].QuoteDetailFields.length;
            j++) {
          if (_quote.QuoteDetail[rowPosition].QuoteDetailFields[j].FieldName ==
              'Quantity') {
            Qty = int.parse(_quote
                .QuoteDetail[rowPosition].QuoteDetailFields[j].FieldValue);
          }
        }
        selectedProductd
            .add(ProductQuantity(_quote.QuoteDetail[rowPosition].product, Qty));
      } else {
        ///REMOVING DESELECTED PRODUCT FROM THE SEND_MAIL_LIST
        selectedProductd.removeWhere((item) =>
            item.ProductObject.ProductCode ==
            _quote.QuoteDetail[rowPosition].product.ProductCode);
        // print('selectedProductd ${selectedProductd.length}');
      }
    });
  }
                  ...
                  Row(
                    children: <Widget>[
                      Checkbox(
                        ...
                        value: _quote.QuoteDetail[qd].product.isSelected,
                        onChanged: (isChecked) {
  ...
  void callAddQuoteAPI({
    ...
    try {
      ...
      _quote.QuoteHeader.forEach((singleQuoteHeader) {
        ...
        //Added by Gaurav,11-09-2020
//        if (widget.listingQuoteObj != null) {
//          if (widget.listingQuoteObj.IsIntegrated.toUpperCase() == "YES") {
//            subHeaderString += '"IsIntegrated" : 1,';
//          } else {
//            subHeaderString += '"IsIntegrated" : 0,';
//          }
//        } else {
//          //Added by Gaurav,14-07-2020
//          subHeaderString += '"IsIntegrated" : 0,';
//        }
        subHeaderString += '"IsIntegrated" : 0,';
      ...
      String _url =
          '${await Session.getData(Session.apiDomain)}${URLs.POST_ADD_QUOTE}';
    ...
    try {
      ...
      _addQuoteHeaderDBHelper
          ...
          .then((value) => {
                ...
                // if (redirectToMain)
                //   {
                ///CALLING QUOTE_DETAIL UPDATE/INSERT
...
//Added By Gaurav 04-03-2022
  ...
  void GetProductRealTimePriceDataForSelectedProduct({
    List<ProductQuantity> selectedProduct,
  }) async {
    try {
      print(isOffline);
      setState(() {
        ...
        try {
          ...
          for (int i = 0; i < selectedProduct.length; i++) {
            if (strFlag == '') {
              ...
              requestBody += '{';
              requestBody +=
                  '"ProductCode" : "${selectedProduct[i].ProductObject.ProductCode}",';
              requestBody += '"Quantity" : "${selectedProduct[i].Quantity}"';
            } else {
              requestBody += ',{';
              requestBody +=
                  '"ProductCode" : "${selectedProduct[i].ProductObject.ProductCode}",';
            ...
            requestBody: requestBody,
              ...
              .then((value) => {
                    if (!value.toString().contains('Error'))
                      {
                        setState(() {
                          ProductRealTimePriceData =
                              List<ProductRealTimePrice>();
                          ProductRealTimePriceData.addAll(value);
                        }),
                        _commonWidgets.showFlutterToast(
                            toastMsg:
                                'Base Price updated succesfully with real time price')
                      }
                    else
                      {
              ...
              .whenComplete(() => {
                    if (ProductRealTimePriceData.length > 0)
                      {
                        AssingRealTimePriceToProduct(ProductRealTimePriceData),
  ...
  void AssingRealTimePriceToProduct(List<ProductRealTimePrice> ProductList) {
    ...
    try {
      ...
      print('Error inside AssingRealTimePriceToProduct FN ');
      print(e);
      setState(() {
        isFullScreenLoading = false;
      });
...
//    return Future.value(true);
  ...
  void startTimer() {
    try {
      // Timer(Duration(seconds: (intervalValue > 1 ? intervalValue ~/ 2 : 1)), () {
      // intialSaveQuote = new Timer(intervalDuration, () {
