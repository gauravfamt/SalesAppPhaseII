...
class CustomerSearchDialogWithoutSearchField extends StatefulWidget {
  ...
  @override
  _CustomerSearchDialogWithoutSearchFieldState createState() =>
      _CustomerSearchDialogWithoutSearchFieldState();
}

class _CustomerSearchDialogWithoutSearchFieldState
    extends State<CustomerSearchDialogWithoutSearchField> {
  ...
  Future<void> getCompanyCount(CustomerSearch type) async {
    try {
      String strWhereClause = " ";
      if (type == CustomerSearch.customerNumber) {
        strWhereClause =
            " and CustomerNo LIKE '${'%' + widget.searchString + '%'}'";
      } else if (type == CustomerSearch.customerName) {
        strWhereClause = " and Name LIKE '${'%' + widget.searchString + '%'}'";
      } else if (type == CustomerSearch.city) {
        strWhereClause = " and City LIKE '${'%' + widget.searchString + '%'}'";
      } else if (type == CustomerSearch.zip) {
        strWhereClause =
            " and PostCode LIKE '${'%' + widget.searchString + '%'}'";
      }
      ...
      print('Last Page Number: $lastPageNumber');
  ...
  Future<List<Company>> fetchOfflineCompaniesData() async {
    ...
    try {
      setState(() {
        getCompanyCount(widget.type);
      });
      if (isLocalDataSynced == true) {
      ...
      // if (isInitialFetch)
      //   showInitialEmptyDataToast(initialFetchCount: companyData.length);
      return companyData;
    } catch (e) {
